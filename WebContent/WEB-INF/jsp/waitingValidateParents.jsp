...
<body style="overflow: scroll; margin: 0px auto;">
			...
			<c:forEach items="${sessionScope.waitingValidateParents }"
				...
				<tr>
					...
					<th style="text-align: center; vertical-align: middle;"><fmt:formatDate
							value="${waitingValidateParent.parentStatuDate}"
							pattern="yyyy-MM-dd HH:mm:ss" /></th>
					<th style="width: 208px; height: 208px;"><a class="preview"
						rel="${pageContext.request.contextPath }${waitingValidateParent.userImage}" href="#"> <img
							src="${pageContext.request.contextPath }${waitingValidateParent.userImage}"
							style="width: 188px; height: 188px;"></a></th>
					<th style="width: 208px; height: 208px;"><a href="#"
						rel="${pageContext.request.contextPath }${waitingValidateParent.idImage}" class="preview"><img
							src="${pageContext.request.contextPath }${waitingValidateParent.idImage}"
							style="width: 188px; height: 188px;"></a></th>
					<th style="text-align: center; vertical-align: middle;">
						<a style="margin-right: 20px;"
						class="btn btn-info" onClick="return confirm('点击确认通过审核？')"
						href="${pageContext.request.contextPath }/admin/checkedSuccess/${waitingValidateParent.id}/家长认证">审核通过</a>
						<a
						class="btn btn-info failBtn" data-toggle="modal"
						data-target="#myModal"
						name="${pageContext.request.contextPath}/admin/checkedFail/${waitingValidateParent.id}">审核不通过</a></th>
				</tr>
			</c:forEach>
		</tbody>
	</table>
	...
	<ul class="pagination">
		<c:if test="${sessionScope.parentsCurrentPage!=1}">
			<li><a
				href="${pageContext.request.contextPath}/admin/findWaitingValidateParents?page=${sessionScope.parentsCurrentPage}&pageAction=pre"
		...
		<c:forEach begin="${sessionScope.parentsBeginPage}"
			...
			<c:if test="${ v==parentsCurrentPage}">
				<li class="active"><a
					href="${pageContext.request.contextPath}/admin/findWaitingValidateParents?page=${v }">${v}</a></li>
			</c:if>
			<c:if test="${ v!=parentsCurrentPage}">
				<li><a
					href="${pageContext.request.contextPath}/admin/findWaitingValidateParents?page=${v }">${v}</a></li>
		...
		<c:if
			...
			<li><a
				href="${pageContext.request.contextPath}/admin/findWaitingValidateParents?page=${sessionScope.parentsCurrentPage}&pageAction=next"
